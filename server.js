...
async function ensureView() {
  ...
  const sql = `
    ...
    SELECT
      ...
      (SELECT numero_lote FROM lotes WHERE produto_id = p.id ORDER BY data_entrada DESC LIMIT 1) AS lote,
      (SELECT validade FROM lotes WHERE produto_id = p.id ORDER BY data_entrada DESC LIMIT 1) AS lote_validade,
      (SELECT quantidade FROM lotes WHERE produto_id = p.id ORDER BY data_entrada DESC LIMIT 1) AS lote_quantidade,
      m.principio_ativo,
      m.dosagem,
